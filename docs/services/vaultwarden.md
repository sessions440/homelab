...
- **2026-08-17:** Upgraded Vaultwarden from 1.36.0 to 1.37.1 via `docker compose pull` and `docker compose up -d` on the `vaultwarden` LXC (`192.168.2.11`). Resolves compatibility issues with Bitwarden 2026.7.0+ desktop/browser extension clients (see `docs/troubleshooting.md`). Verified success from a Bitwarden client.
